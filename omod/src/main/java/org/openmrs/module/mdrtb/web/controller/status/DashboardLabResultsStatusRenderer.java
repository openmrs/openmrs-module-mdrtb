...
public class DashboardLabResultsStatusRenderer implements LabResultsStatusRenderer {
	...
	public void renderSmear(StatusItem item, LabResultsStatus status) {
		...
		if (smear != null) {
			String[] params = {
					MdrtbUtil
							.getConceptName(smear.getResult(), Context.getLocale().getLanguage(), ConceptNameType.SHORT)
							.getName(),
	...
	public void renderPendingLabResults(StatusItem pendingLabResults, LabResultsStatus status) {
		...
		for (StatusItem item : tests) {
			...
			if (testStatus == TestStatus.STARTED) {
				...
				item.setDisplayString(Context.getMessageSourceService().getMessage("mdrtb.labResultsStatus.started",
						params, "{2} started on {1} at {0}", Context.getLocale()));
				...
				item.setDisplayString(Context.getMessageSourceService().getMessage("mdrtb.labResultsStatus.ordered",
						params, "{2} ordered on {1} from {0}", Context.getLocale()));
			} else {
				String[] params = { testType };
				item.setDisplayString(Context.getMessageSourceService().getMessage("mdrtb.labResultsStatus.unknown",
						params, "{0} with status unknown", Context.getLocale()));
			}
		}
	}
	...
	public String renderAnatomicalSite(StatusItem anatomicalStatus) {
		...
		if (anatomicalSite == null) {
			return Context.getMessageSourceService().getMessage("mdrtb.unknown");
				...
				.equals(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.PULMONARY_TB))) {
