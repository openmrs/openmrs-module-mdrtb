...
public class SpecimenMigrationController {
	...
	/**
	 * IMPORTANT NOTE: THESE MIGRATION SCRIPTS SHOULD BE RUN BY A USER WHO HAS HIS/HER DEFAULT
	 * PROFILE LANGUAGE SET TO ENGLISH, SINCE THE MODULE RELIES ON FETCHING CONCEPTS BY ENGLISH NAME
	 * IN ORDER TO WORK
	 */
	...
	public ModelAndView migrateSpecimenData() {
		...
		/**
		 * initialize(); // create all the new concepts/mappings/etc we need to add for the
		 * migration // TODO: NOTE THAT WE WILL WANT TO MAKE SURE WE KEEP CONCEPTS IN SYNC BETWEEN
		 * SERVERS/DICTIONARYS addAndUpdateConcepts(); // convert smears on Haiti forms
		 * convertSmearsOnIntakeAndFollowup(); // migrate any existing Specimen Collection
		 * Encounters in the system // note: this needs to happen first
		 * migrateResultatsDeCrachetEncounters(); // migrate any existing BAC and DST encounters in
		 * the system migrateBacAndDstEncounters(); // clean up old concepts cleanUpConcepts();
		 */
	...
	public ModelAndView addAndUpdateConcepts() {
		...
		addConcept("TUBERCULOSIS TEST DATE ORDERED", "Question", "Date", "TUBERCULOSIS TEST DATE ORDERED",
		    "org.openmrs.module.mdrtb"); // worry about there being two "Date" concept datatypes?
		addConcept("TUBERCULOSIS SPECIMEN COMMENTS", "Finding", "Text", "TUBERCULOSIS SPECIMEN COMMENTS",
		    "org.openmrs.module.mdrtb");
		
		addConcept("PHYSICAN-REPORTED TUBERCULOSIS SMEAR MICROSCOPY CONSTRUCT", "Misc", "N/A",
		    "PHYSICAN-REPORTED TUBERCULOSIS SMEAR MICROSCOPY CONSTRUCT", "org.openmrs.module.mdrtb");
		addConcept("PHYSICAN-REPORTED TUBERCULOSIS SMEAR RESULT", "Test", "Coded",
		    "PHYSICAN-REPORTED TUBERCULOSIS SMEAR RESULT", "org.openmrs.module.mdrtb");
		
		// modify concepts
		
		// haiti-specific?
		// PHYSICAN-REPORTED TUBERCULOSIS SMEAR MICROSCOPY CONSTRUCT
		Concept physicanSmearConstruct = Context.getConceptService()
		        .getConceptByName("PHYSICAN-REPORTED TUBERCULOSIS SMEAR MICROSCOPY CONSTRUCT");
		physicanSmearConstruct.setSet(true);
		physicanSmearConstruct.getConceptSets().add(new ConceptSet(
		        Context.getConceptService().getConcept("PHYSICAN-REPORTED TUBERCULOSIS SMEAR RESULT"), new Double(1)));
		Context.getConceptService().saveConcept(physicanSmearConstruct);
		
		// haiti-specific?
		// PHYSICAN-REPORTED TUBERCULOSIS SMEAR RESULT
		Concept physicanSmearResult = Context.getConceptService()
		        .getConceptByName("PHYSICAN-REPORTED TUBERCULOSIS SMEAR RESULT");
		...
		smearConstruct.getConceptSets()
		        .add(new ConceptSet(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.TEST_DATE_ORDERED),
		                new Double(smearConstruct.getConceptSets().size())));
		smearConstruct.getConceptSets()
		        .add(new ConceptSet(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.TEST_START_DATE),
		                new Double(smearConstruct.getConceptSets().size())));
		...
		cultureConstruct.getConceptSets()
		        .add(new ConceptSet(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.TYPE_OF_ORGANISM),
		                new Double(cultureConstruct.getConceptSets().size())));
		cultureConstruct.getConceptSets()
		        .add(new ConceptSet(
		...
		dstResults.getConceptSets()
		        .add(new ConceptSet(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.DST_CONTAMINATED),
		                new Double(dstResults.getConceptSets().size())));
		dstResults.getConceptSets().add(
		    new ConceptSet(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.WAITING_FOR_TEST_RESULTS),
		            new Double(dstResults.getConceptSets().size())));
		...
		addAnswerConcept(cultureResult,
		    Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.WAITING_FOR_TEST_RESULTS));
	...
	public ModelAndView convertSmearsOnIntakeAndFollowup() {
		...
		Concept smearResult = Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.SMEAR_RESULT);
		Concept physicanSmearConstruct = Context.getConceptService()
		        .getConcept("PHYSICAN-REPORTED TUBERCULOSIS SMEAR MICROSCOPY CONSTRUCT");
			...
			if (encounter.getPatient().isVoided()) {
				...
				for (Obs obs : encounter.getAllObs(false)) {
					if (smearResult.equals(obs.getConcept())) {
						...
						log.info("Changing obs " + obs.getId()
						        + " to type PHYSICAN-REPORTED TUBERCULOSIS SMEAR MICROSCOPY CONSTRUCT");
	...
	public ModelAndView migrateResultatsDeCrachetEncounters() {
		...
		Context.getAdministrationService()
		        .saveGlobalProperty(new GlobalProperty("pihhaiti.dummyMSPPFormId", dummyMSPPForm.getId().toString()));
		...
		for (Encounter encounter : Context.getEncounterService().getEncounters(null, null, null, null, null,
			...
			if (encounter.getPatient().isVoided()) {
				...
				Concept appearanceOfSpecimenType = Context.getService(MdrtbService.class)
				        .getConcept(MdrtbConcepts.SPECIMEN_APPEARANCE);
			...
			if (encounter.getPatient().isVoided()) {
				...
				for (Obs obs : encounter.getAllObs()) {
					if (!StringUtils.isEmpty(obs.getAccessionNumber())) {
						if (specimenMap.get(obs.getAccessionNumber().toUpperCase()) != null
						        && specimenMap.get(obs.getAccessionNumber().toUpperCase()) != specimen) {
							log.warn("Specimen " + specimen.getId() + " and specimen "
							        + specimenMap.get(obs.getAccessionNumber().toUpperCase()).getId()
							        + " may be the same. They share the same accession number.");
						} else {
					...
					if (testConstructConcepts.contains(obs.getConcept())) {
						...
						for (Obs childObs : obs.getGroupMembers()) {
							...
							if (childObs.getConcept().equals(
							    Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.SAMPLE_SOURCE))) {
							...
							if (childObs.getConcept()
								...
								if (!StringUtils.isEmpty(childObs.getAccessionNumber())) {
									log.info("Setting accession number on obs " + obs.getId() + " to "
									        + childObs.getAccessionNumber());
							...
							if (childObs.getConcept().equals(
							    Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.SPUTUM_COLLECTION_DATE))) {
								if (childObs.getValueDatetime() != null) {
									compareAndSetDateCollected(specimen, childObs);
								}
	...
	public ModelAndView voidEncounters() {
		...
		Context.getEncounterService().retireEncounterType(
		    Context.getEncounterService().getEncounterType("Bacteriology Result"), "retired as part of MDR-TB migration");
		Context.getEncounterService().retireEncounterType(Context.getEncounterService().getEncounterType("DST Result"),
		    "retired as part of MDR-TB migration");
	...
	public ModelAndView addWorkflows() {
		...
		afterFailiureCat1
		        .setConcept(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.AFTER_FAILURE_REGIMEN_1));
		...
		afterFailiureCat2
		        .setConcept(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.AFTER_FAILURE_REGIMEN_2));
	...
	public ModelAndView migrateHospitalizations() {
		...
		for (Person patient : Context.getPatientService().getAllPatients(false)) {
			...
			List<Obs> status = Context.getObsService().getObservations(Arrays.asList(patientArray), null,
			    Arrays.asList(conceptParams), null, null, null, null, null, null, null, null, false);
	...
	public ModelAndView createPulmonary() {
		...
		site.addAnswer(
		    new ConceptAnswer(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.EXTRA_PULMONARY_TB)));
	...
	public ModelAndView addDaysToPositivity() {
		addConcept("TUBERCULOSIS CULTURE DAYS TO POSITIVITY", "Question", "Numeric", "DAYS TO POSITIVITY",
		    "org.openmrs.module.mdrtb");
		return new ModelAndView("/module/mdrtb/migration/migration");
	}
				...
				if (location.getId() == null) {
					...
					for (MdrtbPatientProgram program : Context.getService(MdrtbService.class)
						...
						if (program.getLocation() == null) {
							...
							log.info("Set location for patient " + patient.getId() + " and patient program "
							        + program.getId() + " to " + location.getDisplayString());
						}
					}
				}
			}
	...
	public ModelAndView migrateRegistrationGroups() {
		...
		Collection<ProgramWorkflowState> previousTreatmentStates = Context.getService(MdrtbService.class)
		...
		for (Patient patient : Context.getPatientService().getAllPatients()) {
			...
			for (Obs previousTreatmentObs : Context.getObsService().getObservationsByPersonAndConcept(patient,
			    previousTreatment)) {
	...
	public ModelAndView retireOldWorkflows() {
		...
		Concept patientStatus = Context.getConceptService()
		        .getConceptByName("MULTI-DRUG RESISTANT TUBERCULOSIS PATIENT STATUS");
	 ...
	 * instead @RequestMapping("/module/mdrtb/migration/removeWaitingForTestResults.form") public
	 * ModelAndView removeWaitingForTestResults() { // remove Waiting for Test results as optons for
	 * Smear and Culture Results Concept smearResult =
	 * Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.SMEAR_RESULT); Concept
	 * cultureResult =
	 * Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.CULTURE_RESULT); Concept
	 * waitingForTestResults =
	 * Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.WAITING_FOR_TEST_RESULTS);
	 * for (ConceptAnswer answer : smearResult.getAnswers()) { if
	 * (answer.getAnswerConcept().equals(waitingForTestResults)) { smearResult.removeAnswer(answer);
	 * break; } } Context.getConceptService().saveConcept(smearResult); for (ConceptAnswer answer :
	 * cultureResult.getAnswers()) { if (answer.getAnswerConcept().equals(waitingForTestResults)) {
	 * cultureResult.removeAnswer(answer); break; } }
	 * Context.getConceptService().saveConcept(cultureResult); // now void any result obs that are
	 * set to WAITING FOR TEST RESULTS Concept [] concepts = {smearResult, cultureResult}; Concept
	 * [] answers = {waitingForTestResults}; List<Obs> waitingObs =
	 * Context.getObsService().getObservations(null, null, Arrays.asList(concepts),
	 * Arrays.asList(answers), null, null, null, null, null, null, null, false); for (Obs obs :
	 * waitingObs) { Context.getObsService().voidObs(obs, "voided as part of mdr-tb migration"); }
	 * return new ModelAndView("/module/mdrtb/migration/migration"); }
	 */
	...
	public ModelAndView createDrugOrderConceptMappings() {
		...
		addConceptMapping("REASON TUBERCULOSIS TREATMENT CHANGED OR STOPPED",
	...
	public ModelAndView setFutureDiscontinuedDateToNull() {
		...
		for (Order order : Context.getOrderService().getOrders()) {
			...
			if (order != null && order.getDiscontinuedDate() != null && order.getDiscontinued()
			        && order.getDiscontinuedDate().after(today)) {
				log.warn("Resetting discontinued state on order " + order.getId()
				        + " to null; discontinued date erronously in the future (" + order.getDiscontinuedDate() + ")");
	...
	public ModelAndView changeStateShortNames() {
		addConceptShortName("CATEGORY 4 TUBERCULOSIS CLASSIFICATION ACCORDING TO RESULT OF PREVIOUS TREATMENT",
		    "Registration Group - Previous Treatment");
		addConceptShortName("CATEGORY 4 TUBERCULOSIS CLASSIFICATION ACCORDING TO PREVIOUS DRUG USE",
	...
	public ModelAndView addZLIdentifiers() {
		...
		for (Patient patient : Context.getPatientService().getAllPatients()) {
			...
			if (!hasZLId) {
				PatientIdentifier identifier = new PatientIdentifier(MdrtbUtil.assignIdentifier(zlIdentifier), zlIdentifier,
				        fixedLocation);
	...
	public ModelAndView closeOpenProgramsWithOutcomes() {
		...
		for (PatientProgram program : Context.getProgramWorkflowService().getPatientPrograms(null, mdrtb, null, null, null,
			...
			if (mdrtbProgram.getOutcome() != null && mdrtbProgram.getOutcome().getConcept() != null
			        && mdrtbProgram.getDateCompleted() == null) {
				PatientState outcomeState = getPatientState(program, outcome);
				mdrtbProgram.setDateCompleted(outcomeState.getStartDate());
				Context.getProgramWorkflowService().savePatientProgram(mdrtbProgram.getPatientProgram());
				log.info("Closed patient program " + program.getId() + " on " + mdrtbProgram.getDateCompleted()
				        + " because program had previously assigned outcome of " + mdrtbProgram.getOutcome());
			}
		}
	...
	public ModelAndView migrateRegistrationGroupStates() {
		...
		Program mdrtb = Context.getService(MdrtbService.class).getMdrtbProgram();
		Concept prevDrugUse = Context.getService(MdrtbService.class)
		        .getConcept(MdrtbConcepts.CAT_4_CLASSIFICATION_PREVIOUS_DRUG_USE);
		Concept prevTreatment = Context.getService(MdrtbService.class)
		        .getConcept(MdrtbConcepts.CAT_4_CLASSIFICATION_PREVIOUS_TREATMENT);
		...
		ProgramWorkflowState newPatientTreatment = prevTreatmentWorkflow
		        .getState(Context.getConceptService().getConcept(1523));
		newPatientTreatment.setConcept(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.NEW));
	...
	public ModelAndView removeOldGlobalProps() {
		String[] GlOBAL_PROPS_TO_REMOVE = { "mdrtb.location_list", "mdrtb.lab_list", "mdrtb.culture_method_concept",
		        "mdrtb.DST_methods", "mdrtb.smear_method_concept", "mdrtb.anatomical_locations_concept",
		        "mdrtb.DST_drug_list", "mdrtb.DST_result_list", "mdrtb.culture_result_list", "mdrtb.smear_result_list",
		        "mdrtb.mdrtb_default_provider", "mdrtb.first_line_drugs", "mdrtb.injectible_drugs", "mdrtb.quinolones",
		        "mdrtb.other_second_line", "mdrtb.discontinue_drug_order_reasons",
		        "mdrtb.default_discontinue_drug_order_reason", "mdrtb.max_num_bacteriologies_or_dsts_to_add_at_once",
		        "mdrtb.organism_type", "mdrtb.positive_culture_concepts", "mdrtb.ART_identifier_type",
		        "mdrtb.dst_color_coding_red", "mdrtb.dst_color_coding_green", "mdrtb.dst_color_coding_yellow",
		        "mdrtb.in_mdrtb_program_cohort_definition_id", "mdrtb.conversion_definition_interval",
		        "mdrtb.conversion_definition_number", "mdrtb.red_list", "mdrtb.green_list", "mdrtb.yellow_list",
			...
			if (patient.getDead()) {
				...
				for (MdrtbPatientProgram program : Context.getService(MdrtbService.class).getMdrtbPatientPrograms(patient)) {
					...
					if (program.getCurrentlyHospitalized()) {
						...
						Context.getProgramWorkflowService().savePatientProgram(program.getPatientProgram());
						log.info("Closed hospitalization for deceased patient " + patient.getId() + " on "
	...
	public ModelAndView voidPersonsAssociatedWithVoidedPatients() {
		for (Patient patient : Context.getPatientService().getAllPatients(true)) {
			if (patient.isVoided() && !patient.isPersonVoided()) {
				...
				Context.getPatientService().savePatient(patient);
				log.info("Voiding person " + patient.getPersonId() + " because associated patient " + patient.getId()
				        + " has been voided");
			}
		}
	...
	private void createHospitalization(List<MdrtbPatientProgram> programs, Person patient, Date admissionDate,
	        Date dischargeDate) {
		...
		for (MdrtbPatientProgram program : programs) {
			if (program.getDateCompleted() == null || admissionDate.before(program.getDateCompleted())
				...
				log.info("Creating hospitalization for patient # " + patient.getId() + " from " + admissionDate + " to "
				        + dischargeDate);
				Context.getProgramWorkflowService().savePatientProgram(program.getPatientProgram());
				return;
			}
		...
		log.error("Unable to add hospitalization for patient # " + patient.getId() + " from " + admissionDate + " to "
		        + dischargeDate);
	}
	...
	private void compareAndSetSampleSource(Specimen specimen, Obs obs) {
		...
		if (type.getValueCoded() != obs.getValueCoded()) {
			log.warn("Mismatched sample type on specimen " + specimen.getId()
			        + "; using sample source with most recent obs date");
		}
		
		if (type.getObsDatetime() == null
		        || (obs.getObsDatetime() != null && type.getObsDatetime().before(obs.getObsDatetime()))) {
	...
	private Boolean compareAndUpdateColonies(Obs source, Obs target) {
		...
		if (target.getObsDatetime() == null
		        || (source.getObsDatetime() != null && source.getObsDatetime().before(target.getObsDatetime()))) {
