...
public class HAINImpl extends TestImpl implements HAIN {
	...
	public HAINImpl(Obs hain) {
		...
		if(hain == null || !(hain.getConcept().equals(Context.getService(TbService.class).getConcept(TbConcepts.HAIN_CONSTRUCT)))) {
	...
	public HAINImpl(Encounter encounter) {
		...
		test = new Obs (encounter.getPatient(), Context.getService(TbService.class).getConcept(TbConcepts.HAIN_CONSTRUCT), encounter.getEncounterDatetime(), null);
	}
	...
	public Concept getInhResistance() {
		Obs obs = TbUtil.getObsFromObsGroup(Context.getService(TbService.class).getConcept(TbConcepts.ISONIAZID_RESISTANCE), test);
	...
	public void setInhResistance(Concept inhResistance) {
		...
		// initialize the obs if needed
		if (obs == null) {		
			obs = new Obs (test.getPerson(), Context.getService(TbService.class).getConcept(TbConcepts.ISONIAZID_RESISTANCE), test.getObsDatetime(), test.getLocation());
			obs.setEncounter(test.getEncounter());
			test.addGroupMember(obs);
		}
