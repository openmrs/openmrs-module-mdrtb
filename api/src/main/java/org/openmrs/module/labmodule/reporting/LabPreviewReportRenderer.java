 ...
 */
@Handler
@Localized("dotsreports.reporting.PreviewReportRenderer")
public class LabPreviewReportRenderer extends SimpleHtmlReportRenderer {
				...
				if (dataset.getDefinition() instanceof CohortCrossTabDataSetDefinition) {
					...
					if (rows.isEmpty()) {
						...
						for (String rowName : rows) {
							...
							for (String colName : cols) {
								w.write("<td>");
								//Dirty Hack for DST report
								//if(colName.equalsIgnoreCase("PercentNew") || colName.equalsIgnoreCase("PercentPrev") || colName.equalsIgnoreCase("PercentTTL"))
								//{
//									Integer numerator = 0;
//									Integer denominator = 0;
//									outputColumnValue(w, rowName + " - " + colName, "omar");
//								}
//								else{
