...
public class XpertImpl extends TestImpl implements Xpert {
	...
	public Concept getMethod() {
		Obs obs = MdrtbUtil.getObsFromObsGroup(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.GENEXPERT),
		    test);
	...
	public void setMethod(Concept method) {
		...
		// initialize the obs if needed
		if (obs == null) {
			obs = new Obs(test.getPerson(), Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.GENEXPERT),
			        test.getObsDatetime(), test.getLocation());
			obs.setEncounter(test.getEncounter());
			test.addGroupMember(obs);
		}
