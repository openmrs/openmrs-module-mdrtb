...
/**
 * An implementaton of a MdrtbSmear. This wraps an ObsGroup and provides access
 * to smear data within the obsgroup.
 */
public class SmearImpl extends BacteriologyImpl implements Smear {

	public SmearImpl() {
		test = new Obs(null, Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.SMEAR_CONSTRUCT), null,
				null);
	}
	...
	public SmearImpl(Obs smear) {
		...
		if (smear == null || !(smear.getConcept()
	...
	public Integer getBacilli() {
		Obs obs = MdrtbUtil.getObsFromObsGroup(Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.BACILLI),
				test);
	...
	public void setMethod(Concept method) {
		...
		if (obs == null) {
			obs = new Obs(test.getPerson(),
					Context.getService(MdrtbService.class).getConcept(MdrtbConcepts.SMEAR_METHOD),
					test.getObsDatetime(), test.getLocation());
			obs.setEncounter(test.getEncounter());
			test.addGroupMember(obs);
		}
